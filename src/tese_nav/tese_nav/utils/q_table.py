...
class QTable:
    ...
    def __init__(self, alpha: float = 0.1, gamma: float = 0.95):
        ...
        self.updates = 0
        # soma dos |td_error| por janela, para medir convergência
    ...
    def update(self, state: State, action: str, reward: float,
        ...
        self._q[(state, action)] = q_sa + self.alpha * td_error + eta * phi
    ...
    def mean_abs_td(self, reset: bool = True) -> float:
        """TD-error médio absoluto da janela (proxy de convergência)."""
